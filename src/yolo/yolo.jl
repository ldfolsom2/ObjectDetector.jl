...
const weights_dir = joinpath(@__DIR__, "weights")
...
function __init__()
    ...
    CU_FUNCTIONAL[] = CUDAnative.functional()
end
...
YOLOv4 wants a mish activation function so we provide it
"""
mish(x) = x * tanh(log1p(exp(-abs(x))) + max(x, 0))
CuArrays.@cufunc mish(x) = x * tanh(log1p(exp(-abs(x))) + max(x, 0))
...
isn't supported by CuDNN
"""
function spp(x, kernel = [2,2,2])
    x1 = maxpool(x, PoolDims(x, (kernel[1], kernel[1]); padding = floor(kernel[1]/2), stride = 1))
    x2 = maxpool(x, PoolDims(x, (kernel[2], kernel[2]); padding = floor(kernel[2]/2), stride = 1))
    x3 = maxpool(x, PoolDims(x, (kernel[3], kernel[3]); padding = floor(kernel[3]/2), stride = 1))
    return cat(x1,x2,x3,dims=3)
end
...
mutable struct yolo <: AbstractModel
    ...
    chain_layers::Array{Any,1}             # This holds the layer numbers contained in each chain
    W::Dict{Int64, T} where T <: DenseArray  # This holds arrays that the model writes to (1 value per chain)
    out::Array{Dict{Symbol, Any}, 1}         # This holds values and arrays needed for inference
    activations::Dict{Int64,T} where T <: DenseArray   # This holds the activations from the desired layers (if available)
    layers_out::Array{Int64,1}                         # Activations from this layer are placed in yolo.activations

    # The constructor takes the official YOLO config files and weight files
    yolo(cfgfile::String, weightfile::String, batchsize::Int = 1; silent::Bool = false, cfgchanges=nothing, layers_out=[133]) = begin  # Layer 103+1 for v3, Layer 132+1 for v4
        ...
        fn = Array{Any, 1}(nothing, 0) # this keeps the 'function' generated by every layer [akin to functionalized version of each layer, same length as #layers]
        fn_layers = Array{Int64,1}() # this keeps tabs on which functions correspond with each layer
        for (i_layer, (blocktype, block)) in enumerate(cfgvec[2:end])
            ...
            elseif blocktype == :route
                ...
                # @info "route indices" i_layer, idx1, idx2
                !silent && prettyprint(["\n($(length(fn))) ","route($idx1,$idx2)"," => "],[:blue,:cyan,:green])
            elseif blocktype == :shortcut #eg. ResNEt
                act = ACT[block[:activation]]
            ...
            elseif blocktype == :spp
                siz1,siz2,siz3 = block[:size][:]
                stride = block[:stride]
                if stride==1 && CU_FUNCTIONAL[]
                    push!(fn, let; _spp(x) = spp(x, [siz1,siz2,siz3]) end) #Asymmetric padding not supported by CuDNN
                else
                    @info "Incorrect SPP stride or CU_FUNCTIONAL[] is false"
                end
                push!(fn_layers, i_layer)
                push!(ch, ch[end] + ch[end] + ch[end])
                !silent && prettyprint(["($(length(fn))) ","spp([$siz1,$siz2,$siz3],$stride)"," => "],[:blue,:magenta,:green])
            end
        end
        ...
        chainstack_layers = Array{Any,1}() # layers used in each chain
        ...
        for i in 2:length(needout)
            ...
            push!(chainstack_layers, (fn_layers[fst:lst]) ) # layers that constitute the the chain
        ...
        @views for i in eachindex(out)
            ...
            if CU_FUNCTIONAL[]
                offset = reshape(CuArrays.zeros(Float32, w*h*2*length(anchormask)*b), w, h, 2, length(anchormask), b)
            else
            ...
            out[i][:scale] = scale
            out[i][:anchor] = anchor
            out[i][:truth] = get(cfg[:output][i], :truth_thresh, get(cfg[:output][i], :thresh, 0.0)) # for object being detected (at all). Called thresh in v2 [truth_thresh=objectness] (defaults to :thresh. If :thresh not provided, defaults to 0.0)
            out[i][:ignore] = get(cfg[:output][i], :ignore_thresh, 0.3) # for ignoring detections of same object (overlapping) [ignore_thresh=overlap=non-max suppression within-class] (defaults to 0.3 if not provided)
        end

        # set activations to track output of characteristic chains
        activ_type=[typeof(W[key]) for key in keys(W)][1]
        activations = Dict{Int64,activ_type}()
        for i_layer in layers_out
            idx = findall(x->x[end]==layers_out[1],chainstack_layers)
            length(idx)>0 ? activations[i_layer]=W[idx[1]] : activations[i_layer]=DenseArray{Float32,4}()
            # will use pointers to put same data in activations as W
        end

        return new(cfg, chainstack, chainstack_layers, W, out, activations, layers_out)
    end
end
...
function clipdetect!(input::Array, conf)
   ...
   for i in 1:cols
       input[5, i] = ifelse(input[5, i] > conf, input[5, i], Float32(0.0))
   end
end
...
function kern_genbools(input::CuDeviceArray, output::CuDeviceArray)
    ...
    cols = gridDim().x
    if col < cols && input[5, col] > Float32(0.0)
...
function extend_for_attributes(weights::CuArray, w, h, bo, ba)
    return cat(weights, CuArrays.zeros(Float32, w, h, 4, bo, ba), dims = 3)
end
...
detectThresh: Optionally override the minimum allowable detection confidence (default is nothing, ==objectness)
overalThresh: Optionally override the maximum allowable overlap (IoU) (default is effectively 0.3)
"""
function (yolo::yolo)(img::DenseArray; detectThresh=nothing, overlapThresh=yolo.out[1][:ignore])
    ...
    # FORWARD PASS
    ##############
    # Access the correct index of W when a YOLO layer occurs
    no_key = 0
    for i in keys(yolo.chain) # each chain writes to a predefined output
        if haskey(yolo.W,i+no_key)
            yolo.W[i+no_key] .= yolo.chain[i](yolo.W[i+no_key-1])
        else
            no_key += 1
            continue
        end
    ...
    # removes detections already flagged for removal
    #   -ie remove those with low objectness (clipdetect!)
